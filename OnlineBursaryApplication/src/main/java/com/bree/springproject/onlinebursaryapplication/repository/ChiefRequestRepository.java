...
public interface ChiefRequestRepository extends CrudRepository<ChiefDataEntity, Long> {
    ...
    * 2. Loading the form to the chief.*/
    ...
    @Query("SELECT c.userValuesId, c.chiefUserName, c.userId, c.bursaryMonth, c.status FROM ChiefDataEntity c WHERE " +
            "c.chiefUserName= :chiefUserName AND c.bursaryMonth = :bursaryMonth AND c.status = false")
